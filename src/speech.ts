...
import type { OperationsMonitor } from './operations.js';
...
export class Speech implements SpeechService {
  ...
  constructor(private readonly c: Config, private readonly fetcher: Fetcher = fetch, private readonly monitor?: Pick<OperationsMonitor, 'noteSpeechResult'>) {
  ...
  async transcribe(audio: Buffer, mimeType: string, filename: string): Promise<string> {
    ...
    if (this.c.GROQ_API_KEY) {
      try {
        ...
        const text = z.object({ text: z.string() }).parse(raw).text;
        await this.report('stt', true);
        return this.validTranscript(text);
      } catch (error) {
        if (error instanceof UserError) throw error;
        if (!this.c.GEMINI_API_KEY) {
          await this.report('stt', false);
  ...
  async speak(text: string): Promise<Buffer> {
    ...
    if (this.c.GROQ_API_KEY && Date.now() >= this.remoteSpeechRetryAt) {
      try {
        const voice = await this.remoteSpeech(text);
        await this.report('tts-remote', true);
        await this.report('tts', true);
        return voice;
      }
      catch (error) {
        this.remoteSpeechRetryAt = Date.now() + 10 * 60_000;
        await this.report('tts-remote', false);
        if (this.c.LOCAL_TTS_FALLBACK !== 'true') { await this.report('tts', false); throw error; }
      }
    }
    if (this.c.LOCAL_TTS_FALLBACK !== 'true') { await this.report('tts', false); throw new UserError('Озвучка временно недоступна.'); }
    try {
      const voice = await wavsToVoice([await localSpeech(text)], ffmpegPath(this.c.FFMPEG_PATH));
      await this.report('tts', true);
      return voice;
    } catch (error) { await this.report('tts', false); throw error; }
  }

  private async report(feature: 'stt' | 'tts' | 'tts-remote', success: boolean): Promise<void> {
    try { await this.monitor?.noteSpeechResult(feature, success); } catch { /* Telemetry must not suppress a usable response. */ }
  }
