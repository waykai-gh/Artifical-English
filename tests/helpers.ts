...
export function testConfig(extra: NodeJS.ProcessEnv = {}) {
  return readConfig({ BOT_TOKEN: `123456:${'x'.repeat(35)}`, DATABASE_URL: 'postgresql://test:test@localhost/test', GROQ_API_KEY: 'test-key', SECURITY_HMAC_KEY: 'test-only-do-not-use-in-production', ...extra });
}
...
export async function testStore(historyTurns = 8) {
  ...
  } }, defaults(testConfig()), historyTurns, 30, 6000, testConfig().SECURITY_HMAC_KEY);
  return { db, store };
}
